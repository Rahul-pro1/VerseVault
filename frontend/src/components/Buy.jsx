...
const Buy = () => {
  ...
  async function handleSubmit(e){
    ...
    try {
      const response = await axios.post(`/api/v1/books/${ id }/buy`, { address, mode_of_payment: payment })
      navigate(`/cart`)
    } catch (error) {
      console.log(error);
    ...
    <div className="bg-gray-900 text-white min-h-screen flex flex-col">
      ...
      >
        ...
        <div className="relative z-10 bg-gray-800 rounded-lg shadow-xl p-8 max-w-md w-full mx-auto">
          ...
          <h2 className="text-4xl font-bold mb-4 text-purple-300">Buy this Book!</h2>
          ...
          <form className="space-y-4" onSubmit={handleSubmit} method='POST'>
            ...
            <Listbox value={payment} onChange={setPayment}>
              <Label className="block text-sm/6 font-medium text-purple-300">Mode of Payment</Label>
              <div className="relative mt-2">
                <ListboxButton className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm/6">
                  <span className="flex items-center">
                    <span className="ml-3 block truncate">{payment}</span>
                  </span>
